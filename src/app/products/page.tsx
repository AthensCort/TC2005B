...
export default function Home() {
  ...
  const [form, setForm] = useState({
    id: null as number | null, // Inicializa id como null
    nombre: "",
    precio: "",
    stock: "",
    photo: "",
  ...
  const handleEdit = (item: Product) => {
    setForm({
      id: item.id || null, // Asigna el ID del producto al formulario
      nombre: item.nombre,
  ...
  const handleSaveEdit = async () => {
    if (form.nombre && form.precio && form.stock && form.photo) {
      ...
      // If `form.photo` is a URL, skip appending it as a file
      if (typeof form.photo === "object") {
        formData.append("photo", form.photo); // Append the file object
      }
      ...
      try {
        ...
        if (isEditing && form.id !== null) {
          ...
          response = await fetch(`http://localhost:8080/api/productoServicio/${form.id}`, {
            method: "PUT", // Use PUT for updates
            body: formData,
          });
        } else {
          // Create new product
          response = await fetch("http://localhost:8080/api/productoServicio", {
            method: "POST", // Use POST for new products
            body: formData,
          });
        }
        ...
        if (isEditing && form.id !== null) {
          ...
          const updatedProducts = product.map((p) =>
            p.id === form.id ? savedProduct : p
          );
          setProduct(updatedProducts);
          setFilteredProducts(updatedProducts);
        } else {
          // Add the new product to the list
          const updatedProducts = [...product, savedProduct];
          setProduct(updatedProducts);
          setFilteredProducts(updatedProducts);
        }
        ...
        setShowModal(false);
        setIsEditing(false);
        setForm({ id: null, nombre: "", precio: "", stock: "", photo: "" });
      } catch (error) {
  ...
  const handleAddNew = () => {
    setForm({ id: null, nombre: "", precio: "", stock: "", photo: "" }); // Reset form for new product, incluyendo id: null
